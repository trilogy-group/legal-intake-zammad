...
RSpec.describe Whatsapp::Webhook::Message::Audio, :aggregate_failures, current_user_id: 1 do
  describe '#process' do
    ...
    context 'when all data is valid' do
      ...
      let(:media_file)     { Tempfile.create('sample.mp3').tap { |f| File.write(f, media_content) } }
      ...
      it 'creates a ticket + an article' do
        ...
        expect(Store.last).to have_attributes(
          filename: "#{Ticket.last.number}-1870770316696531.mp3",
