/**
 * Export current Zammad roles from REST API to local JSON file.
 */
...
async function main(): Promise<void> {
  ...
  console.log("Fetching current roles from Zammad...");
  const roles = await fetchJson<any[]>(
    `${zammadUrl}/api/v1/roles`,
    { headers: authHeaders(token) },
    "GET /api/v1/roles"
  );

  const cleaned = roles.map(
  ...
  writeFileSync(
    outputPath,
    JSON.stringify({ roles: cleaned }, null, 2) + "\n",
    "utf8"
  );
  console.log(`✓ Exported ${roles.length} roles to: ${outputPath}`);
}
