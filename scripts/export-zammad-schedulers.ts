/**
 * Export current Zammad schedulers from REST API to local JSON file.
 */
...
async function main(): Promise<void> {
  ...
  console.log("Fetching current schedulers from Zammad...");
  const schedulers = await fetchJson<any[]>(
    `${zammadUrl}/api/v1/jobs`,
    { headers: authHeaders(token) },
    "GET /api/v1/jobs"
  );

  const cleaned = schedulers.map(
    ({
      id,
      created_at,
      updated_at,
  ...
  writeFileSync(
    outputPath,
    JSON.stringify({ schedulers: cleaned }, null, 2) + "\n",
    "utf8"
  );
  console.log(`✓ Exported ${schedulers.length} schedulers to: ${outputPath}`);
}
