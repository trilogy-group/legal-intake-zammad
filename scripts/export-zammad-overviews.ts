/**
 * Export current Zammad overviews from REST API to local JSON file.
 */
...
async function main(): Promise<void> {
  ...
  console.log("Fetching current overviews from Zammad...");
  const overviews = await fetchJson<any[]>(
    `${zammadUrl}/api/v1/overviews`,
    { headers: authHeaders(token) },
    "GET /api/v1/overviews"
  );

  const cleaned = overviews.map(
  ...
  writeFileSync(
    outputPath,
    JSON.stringify({ overviews: cleaned }, null, 2) + "\n",
    "utf8"
  );
  console.log(`✓ Exported ${overviews.length} overviews to: ${outputPath}`);
}
