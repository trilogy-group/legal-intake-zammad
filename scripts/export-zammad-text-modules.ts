/**
 * Export current Zammad text modules from REST API to local JSON file.
 */
...
async function main(): Promise<void> {
  ...
  console.log("Fetching current text modules from Zammad...");
  const textModules = await fetchJson<any[]>(
    `${zammadUrl}/api/v1/text_modules`,
    { headers: authHeaders(token) },
    "GET /api/v1/text_modules"
  );

  const cleaned = textModules.map(
  ...
  writeFileSync(
    outputPath,
    JSON.stringify({ text_modules: cleaned }, null, 2) + "\n",
    "utf8"
  );
  console.log(
    `✓ Exported ${textModules.length} text modules to: ${outputPath}`
  );
}
