...
import type { BackRoute, UserTaskbarTab } from '#desktop/components/UserTaskbarTabs/types.ts'
...
export const useUserCurrentTaskbarTabsStore = defineStore('userCurrentTaskbarTabs', () => {
  ...
  const taskbarTabList = computed<UserTaskbarTab[]>((currentTaskbarTabList) => {
    ...
    const taskbarTabs: UserTaskbarTab[] = taskbarTabsRaw.value.userCurrentTaskbarItemList
      .filter((taskbarTab) => !taskbarTabIDsInDeletion.value.includes(taskbarTab.id))
      .flatMap((taskbarTab) => {
    ...
    const existingTabEntityKeys = new Set(taskbarTabs.map((taskbarTab) => taskbarTab.tabEntityKey))
  ...
  const activeTaskbarTab = computed<UserTaskbarTab | undefined>((currentActiveTaskbarTab) => {
    if (!activeTaskbarTabEntityKey.value) return

    const newActiveTaskbarTab = taskbarTabList.value.find(
      (taskbarTab) => taskbarTab.tabEntityKey === activeTaskbarTabEntityKey.value,
    )

    if (currentActiveTaskbarTab && isEqual(newActiveTaskbarTab, currentActiveTaskbarTab))
      return currentActiveTaskbarTab

    return newActiveTaskbarTab
  })

  const activeTaskbarTabId = computed(() => activeTaskbarTab.value?.taskbarTabId)

  const activeTaskbarTabEntityAccess = computed(() => activeTaskbarTab.value?.entityAccess)

  const hasTaskbarTabs = computed(() => taskbarTabList.value?.length > 0)

  const taskbarTabListByTabEntityKey = computed(() => keyBy(taskbarTabList.value, 'tabEntityKey'))
  ...
  const redirectToLastHistoricalRoute = () => {
    ...
    const nextRoute = backRoutes.value.find((backRoute) => {
      ...
      if (backRoute.taskbarTabEntityKey === activeTaskbarTabEntityKey.value) return false

      // Check if the taskbar tab route is part of the current taskbar.
      return !!taskbarTabListByTabEntityKey.value[backRoute.taskbarTabEntityKey]
    })
  ...
  const handleActiveTaskbarTabRemoval = (
    ...
    const removedItem = taskbarTabList?.find((tab) => tab.id === removedItemId)
  ...
  >({
    ...
    updateQuery(previous, { subscriptionData }) {
      ...
      if (!previous.userCurrentTaskbarItemList || updates.updateItem) return previous
      ...
      if (updates.removeItem) {
        ...
        handleActiveTaskbarTabRemoval(previousTaskbarTabList, updates.removeItem)
      ...
      if (updates.addItem) {
        ...
        return {
          userCurrentTaskbarItemList: [...previousTaskbarTabList, updates.addItem],
        }
      }
    ...
    useUserCurrentTaskbarItemAddMutation({
      update: (cache, { data }) => {
        ...
        const newIdPresent = taskbarTabList.value.find((taskbarTab) => {
          return taskbarTab.taskbarTabId === userCurrentTaskbarItemAdd.taskbarItem?.id
        })
  ...
  const addTaskbarTab = async (
    ...
    const { buildTaskbarTabEntityId, buildTaskbarTabParams, entityType, entityDocument } =
    ...
    if (entityType && entityDocument) {
      ...
      if (tabEntityInternalId) {
        const cachedEntity = getApolloClient().cache.readFragment<TaskbarItemEntity>({
          id: `${entityType}:${convertToGraphQLId(entityType, tabEntityInternalId)}`,
...
if (import.meta.hot) {
  import.meta.hot.accept(acceptHMRUpdate(useUserCurrentTaskbarTabsStore, import.meta.hot))
}
